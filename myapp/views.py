...
def manage_students(request):
    """List all verified students for teacher management, grouped by section"""
    if not hasattr(request.user, 'profile') or request.user.profile.role != 'teacher':
        messages.error(request, 'Access denied.')
        return redirect('home')
    ...
    ).select_related('profile', 'profile__section').order_by('-date_joined')
    ...
    if section_filter:
        students = students.filter(profile__section_id=section_filter)
    ...
    # Convert paginated students to list for grouping
    students_list = list(page_obj)
    ...
    for student in students_list:
        if student.profile.section:
            section_id = student.profile.section.id
            if section_id not in students_by_section:
                students_by_section[section_id] = {
                    'section': student.profile.section,
                    'students': []
                }
            students_by_section[section_id]['students'].append(student)
        else:
    ...
    # Statistics (for all students, not just current page)
